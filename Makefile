...
CORE_FILES = core_main core_util core_list_join core_matrix core_state 
...
$(OUTFILE): $(SRCS) $(HEADERS) Makefile core_portme.mak $(EXTRA_DEPENDS) $(FORCE_REBUILD)
	...
	$(MAKE) link
	$(MAKE) port_postbuild
	riscv32-unknown-elf-objdump -D -s coremark.bare.riscv > coremark.bare.riscv.dump
	elf2hex --bit-width 32 --input coremark.bare.riscv --output coremark.bare.riscv.hex
...
.PHONY: clean
clean:
	rm -f $(OUTFILE) $(OBJS) $(OPATH)*.log *.info $(OPATH)index.html $(PORT_CLEAN) ./*/*.o
